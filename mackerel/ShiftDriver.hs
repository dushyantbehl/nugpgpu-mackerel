...
device_preamble dev = 
    C.MultiComment [ 
           ...
           "GENERATED BY MACKEREL MODIFIED FOR NUGPGPU",
...
std_header_files dev = 
    map (C.Include C.Standard) c_inclist ++ map (C.Include C.Local) local_inclist
    where 
      local_inclist = [ "../include/mackerel.h" ]
                ++
                [ i ++ ".dev.h" | i <- Dev.imports dev ]
      c_inclist = [ "linux/kernel.h" ]--[ "inttypes.h" ]
